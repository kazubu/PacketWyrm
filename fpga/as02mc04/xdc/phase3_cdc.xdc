...
#
# REGISTER-MERGE ROBUSTNESS: the egress timestamp CDC (u_tscdc) and the new RX
# ingress wire-stamp CDC (u_rxtscdc, see below) both register the SAME function
# bin2gray(ts) in dp_clk, so opt_design merges ALL of them into ONE source cell,
# keeping ONE original name (which of u_tscdc / u_rxtscdc is implementation-
# defined). So the -from filter matches EITHER instance name -- it resolves to
# the single merged source regardless of which name survives -- and -to selects
# the per-port destination sync stage (the real clock-domain split).
# Egress port 0:
set_max_delay -datapath_only \
    -from [get_cells -hier -filter {NAME =~ *u_tscdc/gray_src_reg[*] || NAME =~ *u_rxtscdc/gray_src_reg[*]}] \
...
# RX ingress wire-stamp Gray CDC (pw_ts_gray_cdc as u_rxtscdc, one per port):
# crosses dp_clk -> each MAC sfp_rx_clk. Same Gray-coded scheme as the egress
# CDC above; bound source-reg -> first-sync delay (datapath-only) + inter-bit
# skew so a mid-increment sample resolves to N or N+1, never a far value.
# Per-destination-port split for the same reason as egress (the two RX sync
# stages are in different rx_clk domains -> separate bus-skew groups).
# RX port 0:
set_max_delay -datapath_only \
    -from [get_cells -hier -filter {NAME =~ *u_tscdc/gray_src_reg[*] || NAME =~ *u_rxtscdc/gray_src_reg[*]}] \
    -to   [get_cells -hier -filter {NAME =~ *g_rx_wirestamp[0].u_rxtscdc/sync1_reg[*]}] 6.400
set_bus_skew \
    -from [get_cells -hier -filter {NAME =~ *u_tscdc/gray_src_reg[*] || NAME =~ *u_rxtscdc/gray_src_reg[*]}] \
    -to   [get_cells -hier -filter {NAME =~ *g_rx_wirestamp[0].u_rxtscdc/sync1_reg[*]}] 6.400
# RX port 1:
set_max_delay -datapath_only \
    -from [get_cells -hier -filter {NAME =~ *u_tscdc/gray_src_reg[*] || NAME =~ *u_rxtscdc/gray_src_reg[*]}] \
    -to   [get_cells -hier -filter {NAME =~ *g_rx_wirestamp[1].u_rxtscdc/sync1_reg[*]}] 6.400
set_bus_skew \
    -from [get_cells -hier -filter {NAME =~ *u_tscdc/gray_src_reg[*] || NAME =~ *u_rxtscdc/gray_src_reg[*]}] \
    -to   [get_cells -hier -filter {NAME =~ *g_rx_wirestamp[1].u_rxtscdc/sync1_reg[*]}] 6.400
...
# Same waivers for the RX ingress wire-stamp Gray CDC (u_rxtscdc): dp_clk ->
# each MAC sfp_rx_clk is intentionally async, constrained by the per-port
# set_max_delay -datapath_only + set_bus_skew above. The RX user clock NAMES are
# auto-generated by the GT IP (gtwiz_userclk_rx_*), so derive each rx clock by
# the cells it drives (the per-port u_rxtscdc sync stage) rather than hard-coding
# a name -- robust to GT-IP clock-naming changes. Same CAVEAT as the egress
# waiver: this silences TIMING-6/7 for ANY logic crossing these dp_clk -> rx_clk
# pairs, so vet any future such CDC individually.
create_waiver -type METHODOLOGY -id {TIMING-6} \
    -objects [concat [get_clocks dp_clk_u] [get_clocks -of_objects \
                [get_pins -hier -filter {NAME =~ *g_rx_wirestamp[0].u_rxtscdc/sync1_reg[*]/C}]]] \
    -user pw \
    -description "RX wire-stamp Gray CDC (u_rxtscdc, port 0): dp_clk -> MAC sfp_rx_clk is intentionally async, constrained by set_max_delay -datapath_only + set_bus_skew."
create_waiver -type METHODOLOGY -id {TIMING-7} \
    -objects [concat [get_clocks dp_clk_u] [get_clocks -of_objects \
                [get_pins -hier -filter {NAME =~ *g_rx_wirestamp[0].u_rxtscdc/sync1_reg[*]/C}]]] \
    -user pw \
...
create_waiver -type METHODOLOGY -id {TIMING-6} \
    -objects [concat [get_clocks dp_clk_u] [get_clocks -of_objects \
                [get_pins -hier -filter {NAME =~ *g_rx_wirestamp[1].u_rxtscdc/sync1_reg[*]/C}]]] \
    -user pw \
    -description "RX wire-stamp Gray CDC (u_rxtscdc, port 1): dp_clk -> MAC sfp_rx_clk is intentionally async, constrained by set_max_delay -datapath_only + set_bus_skew."
create_waiver -type METHODOLOGY -id {TIMING-7} \
    -objects [concat [get_clocks dp_clk_u] [get_clocks -of_objects \
                [get_pins -hier -filter {NAME =~ *g_rx_wirestamp[1].u_rxtscdc/sync1_reg[*]/C}]]] \
    -user pw \
